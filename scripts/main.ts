...
let clickedPlaceStateId = null;
const popup = new mapboxgl.Popup()

function setNameAndHint(e: mapboxgl.MapMouseEvent & { features?: mapboxgl.MapboxGeoJSONFeature[]; } & mapboxgl.EventData) {
    const expectedName = e.features[0].properties.name;
...
function setMouseEnterAndLeave(source: string | readonly string[]) {
    ...
    if (map.getLayer('plymouth_places') === undefined) {
        ...
        map.on('click', 'plymouth_places', (e) => {
            ...
            if (clickedPlaceStateId !== null) {
                map.setFeatureState(
                    { source: 'plymouth_places', id: clickedPlaceStateId },
                    { clicked: false }
                );
            }
...
map.on('load', () => {
    ...
    const streetCheckBox = document.getElementById('streets');
